...
# Fallback font names for special characters - ordered by Unicode coverage
FALLBACK_FONTS = [
    "Noto Sans",
    "Arial Unicode MS", 
    "DejaVu Sans",
...
# Cache for font support testing
_FONT_CACHE = {}


def get_best_font_for_text(text: str, font_size: int, preferred_font: skia.Font) -> skia.Font:
    """Get the best font that supports the most characters in the text.
    
    Strategy:
    1. Test if preferred font supports ALL characters -> use it (fastest)
    2. Test each fallback font to find one that supports ALL characters
    3. If none support all, use the one that supports the most characters
    """
    glyphs = preferred_font.textToGlyphs(text)
    
    # Quick check: if all characters are supported, use preferred font
    missing_count = sum(1 for g in glyphs if g == 0)
    if missing_count == 0:
        return preferred_font
    
    logger.debug(f"Preferred font missing {missing_count}/{len(glyphs)} glyphs")
    
    # Try fallback fonts
    best_font = preferred_font
    best_missing = missing_count
    
    for font_name in FALLBACK_FONTS:
        try:
            # Use cache to avoid recreating fonts
            cache_key = (font_name, font_size)
            if cache_key not in _FONT_CACHE:
                typeface = skia.Typeface(font_name) if font_name else skia.Typeface()
                _FONT_CACHE[cache_key] = skia.Font(typeface, font_size)
            
            fallback_font = _FONT_CACHE[cache_key]
            fallback_glyphs = fallback_font.textToGlyphs(text)
            fallback_missing = sum(1 for g in fallback_glyphs if g == 0)
            
            # If this font supports all characters, use it immediately
            if fallback_missing == 0:
                logger.info(f"Using fallback font '{font_name or 'default'}' (all glyphs supported)")
                return fallback_font
            
            # Track the best font so far
            if fallback_missing < best_missing:
                best_font = fallback_font
                best_missing = fallback_missing
                
        except Exception as e:
            logger.debug(f"Fallback font '{font_name}' failed: {e}")
            continue
    
    # Use the best font we found
    if best_missing < missing_count:
        logger.info(f"Using fallback font with {best_missing} missing glyphs (vs {missing_count})")
    else:
        logger.warning(f"No better fallback found, {best_missing} glyphs still missing")
...
def make_text_blob(
    ...
    preferred_font = skia.Font(typeface, int(element.fontSize))
    
    # Smart fallback: find best font for this text
    font = get_best_font_for_text(text, int(element.fontSize), preferred_font)
